...
from typing import Optional, List  # Remove unused Dict, Union imports
...
from .defaults import (
    ...
    MODEL_PATHS,
    DATA_DEFAULTS,
    OPTIMIZER_DEFAULTS,  # Add this import
)
...
class ModelConfig(BaseConfig):
    ...
    DEFAULT_LR = OPTIMIZER_DEFAULTS["lr"]  # Changed from learning_rate
    DEFAULT_DEVICE = MODEL_DEFAULTS["device"]
    DEFAULT_DROPOUT_RATE = CLASSIFIER_DEFAULTS[
        "dropout_rate"
    ]  # Changed from hidden_dropout
    ...
    num_classes: Optional[int] = CLASSIFIER_DEFAULTS[
        "num_classes"
    ]  # Added default from CLASSIFIER_DEFAULTS
    ...
    metrics: List[str] = field(default_factory=lambda: MODEL_DEFAULTS["metrics"])
    ...
    def _init_directories(self):
        ...
        config_paths = [
            ...
            Path(
                os.environ.get("BERT_DIR_CONFIG", "config/directories.yml")
            ),  # Fallback
        ]
        ...
        try:
            if config_file:
                ...
                output_root = Path(
                    dir_config.get("output_root", DIR_DEFAULTS["output_root"])
                )
                dirs = dir_config.get("dirs", DIR_DEFAULTS["dirs"])
                model_paths = dir_config.get(
            ...
            logger.warning("Failed to load directory config: %s", e)
    ...
    def _validate_metrics(self, metrics: List[str]) -> None:
        ...
        if invalid_metrics:
            raise ValueError(
                f"Invalid metrics: {invalid_metrics}. Valid options are: {VALID_METRICS}"
            )
    ...
    def _validate_metric(self, metric: str) -> None:
        ...
        if metric not in VALID_METRICS:
            raise ValueError(
                f"Primary metric '{metric}' must be one of {VALID_METRICS}"
            )
    ...
    def add_argparse_args(cls, parser: argparse.ArgumentParser) -> None:
        ...
        training.add_argument(
            "--num_epochs",
            type=int,
            default=cls.DEFAULT_NUM_EPOCHS,
        ...
        training.add_argument(
            "--learning_rate",  # Keep CLI argument name for backward compatibility
            type=float,
            default=cls.DEFAULT_LR,  # Changed from DEFAULT_LEARNING_RATE to DEFAULT_LR
            help="Learning rate",
        )
        training.add_argument(
            "--dropout_rate",
            type=float,  # Changed from hidden_dropout
            default=cls.DEFAULT_DROPOUT_RATE,
            help="Dropout rate for neural network layers",
        )
        ...
        model.add_argument(
            "--bert_model_name",
            type=str,
            default=cls.bert_model_name,
            help="Name or path of the pre-trained BERT model",
        )
        model.add_argument(
            "--num_classes",
            type=int,
            default=cls.num_classes,
            help="Number of output classes",
        )
        model.add_argument(
            "--max_seq_len",
            type=int,
            default=cls.DEFAULT_MAX_SEQ_LEN,  # Changed from max_length to max_seq_len
        ...
        paths.add_argument(
            "--data_file",
            type=Path,
            default=cls.DEFAULT_DATA_FILE,
            help="Path to input data file",
        )
        # Remove default value for model_save_path, will be set based on output_root
        paths.add_argument(
            "--model_save_path", type=Path, help="Path to save the trained model"
        )
        ...
        experiment.add_argument(
            "--n_experiments",
            type=int,
            default=cls.n_experiments,
            help="Number of experiments to run",
        )
        experiment.add_argument(
            "--trials_per_experiment",
            type=int,
            default=cls.trials_per_experiment,
...
class ValidationConfig(ModelConfig):
    ...
    def add_argparse_args(cls, parser: argparse.ArgumentParser) -> None:
        ...
        validation.add_argument(
            "--output_dir", type=Path, help="Directory to save validation results"
        )  # Remove default
        validation.add_argument(
            "--threshold",
            type=float,
            default=cls.threshold,
    ...
    def _resolve_paths(self) -> None:
        ...
        if self.model_path is None:
            raise ValueError(
                "No model path specified and no best model found in best_trials_dir"
            )
            ...
            for file in model_files:
                try:
                    ...
                    if score > best_score:
                        ...
                        best_model = file
                        logger.info(
                            "Found better model with score %.4f: %s", score, file
                        )
                except (
                    IOError,
                    RuntimeError,
...
class EvaluationConfig(ModelConfig):
        ...
        if self.best_model and not self.best_model.is_absolute():
            ...
            if best_model_path.exists():
                ...
                if best_model_path.exists():
                    ...
                    logger.warning(
                        "Best model not found at either:\n  %s\n  %s",
                        best_model_path,
                        self.best_trials_dir / self.best_model.name,
                    )
    ...
    def add_argparse_args(cls, parser: argparse.ArgumentParser) -> None:
        ...
        eval_group.add_argument(
            "--output_dir", type=Path, help="Directory to save evaluation results"
        )
...
def load_best_configuration(
    ...
    if best_trial:
        logger.info("Best trial score: %.4f", best_value)  # Use best_value directly
